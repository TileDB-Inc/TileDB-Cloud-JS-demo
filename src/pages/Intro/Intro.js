  ...
  return (
    <div>
      ...
      <Tabs defaultActiveKey="1">
        <TabPane tab="Reads" key="1">
          <Title level={3}>Basic read query</Title>
          <Paragraph>
            Dimensions should always be an array of 2 (start of the range and
            the end of the range). <b>bufferSize</b> dictates the server the
            number of bytes that should allocated to make this query. In case
            the bufferSize is not enough, it will result to an incomplete query.
            For this reason ReadQuery is an async generator so a user could get
            results in batches.
          </Paragraph>
          ...
          <Paragraph>
            By setting a dimension as an empty array, query will select the
            whole dimension.
          </Paragraph>
          <CodeSnippet>{whole_dim}</CodeSnippet>
        </TabPane>
        <TabPane tab="Writes" key="2">
